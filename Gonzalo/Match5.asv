function M = Match4(Temp,dTemp,Res,num_Patron,Temp_Baño)
...
Patron=R0*(1+a*(Temp_Baño-To)+b*(Temp_Baño-To).^2);
uPatron=R0*(a*u+2*b*u*(Temp_Baño-To));
...
%[tiempo de medicion,temepratura,valor de resistencia,incerteza del valor de resistencia, derivadas de la temperatura]
if LT <= LR
    for i = 1:LT
      for j=1:LR-1;
    ...
    for i = 1:LR
      for j=1:LT-1;
        if Res(i,1) >= Temp(j,1) && Res(i,1) < Temp(j+1,1)
            ...
            M(k,:) = [Res(i,1),Interpolacion_Temperatura(j,2:A_T),Res(i,q)*Patron,Res(i,q)*uPatron,Interpolacion_dTemperatura(j,2:A_dT)];
            k=k+1;
        end
      end
    end
