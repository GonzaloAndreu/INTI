function M = Match4(Temp,dTemp,Res,t,To,P,a1,b1,r)
T=t;
if r==2
      ...
      for j=1:LR-1;
        if Temp(i,1) > Res(j,1) && Temp(i,1) < Res(j+1,1)
          if (Temp(i,1)-Res(j,1)) <= (Res(j+1,1)-Temp(i,1))
            M(k,:) = [Temp(i,1),Temp(i,2),Res(j,q)*Patron,dTemp(i,2),Temp(i,2)^4,Temp(i,3),dTemp(i,3)]
          end
          if (Temp(i,1)-Res(j,1)) > (Res(j+1,1)-Temp(i,1))
            M(k,:) = [Temp(i,1),Temp(i,2),Res(j+1,q)*Patron,dTemp(i,2),Temp(i,2)^4,Temp(i,3),dTemp(i,2)]
          end
          k=k+1;
        end
      end
